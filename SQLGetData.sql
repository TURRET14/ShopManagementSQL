...
AS BEGIN
IF (Dbo.SignIn(@AdminLogin, @AdminPassword) NOT IN ('Администратор'))
...
@AdminPassword NVARCHAR(50)
AS BEGIN
IF (Dbo.SignIn(@AdminLogin, @AdminPassword) NOT IN ('Администратор', 'Менеджер', 'Кассир'))
   THROW 50000, 'AUTHORIZATION_ERROR', 255;
...
@AdminPassword NVARCHAR(50)
AS BEGIN
IF (Dbo.SignIn(@AdminLogin, @AdminPassword) NOT IN ('Администратор', 'Менеджер'))
   THROW 50000, 'AUTHORIZATION_ERROR', 255;
